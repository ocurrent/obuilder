...
module Make_internal (Raw_store : S.STORE) (Sandbox : S.SANDBOX) (Fetch : S.FETCHER) = struct
  ...
  let run t ~switch ~log ~cache run_input kont =
    let id =
  ...
  let copy t ~context ~base { Obuilder_spec.from; src; dst; exclude } kont =
    ...
    kont t ~switch ~base ~log ~exclude ~src_dir ~dst src user
  ...
  let rec run_steps t ~(context:Context.t) ~base run_kont copy_kont = function
    | [] -> Lwt_result.return base
    | op :: ops ->
      context.log `Heading Fmt.(strf "%a" (pp_op ~context) op);
      let k = run_steps t run_kont copy_kont ops in
      match op with
      ...
      | `Run { shell = cmd; cache; network; secrets = mount_secrets } ->
        ...
        run t ~switch ~log ~cache run_input run_kont >>!= fun base ->
        k ~base ~context
      | `Copy x ->
        copy t ~context ~base x copy_kont >>!= fun base ->
        k ~base ~context
      | `Env ((key, _) as e) ->
  ...
  let rec build ~scope t context { Obuilder_spec.child_builds; from = base; ops } get_base_kont run_kont copy_kont =
    let rec aux context = function
      ...
      | (name, child_spec) :: child_builds ->
        ...
        build ~scope t context child_spec get_base_kont run_kont copy_kont >>!= fun child_result ->
    ...
    get_base t ~log:context.Context.log base get_base_kont >>!= fun (id, env) ->
    let context = { context with env = context.env @ env } in
    run_steps t ~context ~base:id run_kont copy_kont ops

  let build t context spec get_base_kont run_kont copy_kont =
    let r = build ~scope:[] t context spec get_base_kont run_kont copy_kont in
  ...
  let healthcheck ?(timeout=30.0) t get_base_kont run_kont copy_kont =
    ...
    get_base t ~log healthcheck_base get_base_kont >>= function
    ...
    | Ok (id, env) ->
      ...
      run_steps t ~context ~base:id run_kont copy_kont healthcheck_ops >>= function
...
module Make (Raw_store : S.STORE) (Sandbox : S.SANDBOX) (Fetch : S.FETCHER) = struct
  module Build = Make_internal (Raw_store) (Sandbox) (Fetch)
  module Store = Build.Store
  type t = Build.t

  let delete ?log t id =
    Build.delete ?log t id

  let prune ?log t ~before limit =
    Build.prune ?log t ~before limit

  let get_base_kont _base _id = Lwt_result.return ()

  let run_kont _id = Lwt.return_unit

  let copy_kont t ~switch ~base ~log ~exclude ~src_dir ~dst src user =
    let src_dir = match src_dir with `Context src_dir -> src_dir | `Build (_, src_dir) -> src_dir | _ -> assert false in
    ...
    | Ok op ->
      ...
      Log.debug (fun f -> f "COPY: %a@." Sexplib.Sexp.pp_hum (Build.sexp_of_copy_details details));
      let id = Sha256.to_hex (Sha256.string (Sexplib.Sexp.to_string (Build.sexp_of_copy_details details))) in
      Store.build t.Build.store ?switch ~base ~id ~log (fun ~cancelled ~log result_tmp ->
          let argv = ["tar"; "-xf"; "-"] in
          let config = Config.v
              ~cwd:"/"
              ~argv
  ...
  let build t context spec =
    Build.build t context spec get_base_kont run_kont copy_kont

  let healthcheck ?timeout t =
    Build.healthcheck ?timeout t get_base_kont run_kont copy_kont

  let v = Build.v
end

module MakeDocker (Raw_store : S.STORE) (Sandbox : S.SANDBOX) (Fetch : S.FETCHER) = struct
  module Build = Make_internal (Raw_store) (Sandbox) (Fetch)
  module Store = Build.Store
  type t = Build.t
  ...
  let run_kont id = docker_teardown_sandbox id ~commit:true

  let get_base_kont base id =
  ...
  let docker_tarball_from_build t ~log ~dst ~from (`Docker_volume volume) mount_point op user =
    ...
    Lwt.finalize
      (fun () ->
        Lwt_result.bind_lwt
          (Sandbox.run ~cancelled ~log t.Build.sandbox config from)
